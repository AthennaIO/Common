...
export default class PathTest {
  ...
  public async shouldGetTheSubPathsOfSrcMainPath({ assert }: Context) {
    ...
    assert.equal(Path.mcp(), mainPath.concat(sep, 'mcp'))
  ...
  public async shouldBeAbleToSetupDifferentPathsForAllMethodsOfPathClass({ assert }: Context) {
    Path.setBin('build/bin')
      ...
      .setMcp('build/src/mcp')
      .setTools('build/src/mcp/tools')
    ...
    assert.isTrue(Path.mcp().endsWith(`build${sep}src${sep}mcp`))
    assert.isTrue(Path.tools().endsWith(`build${sep}src${sep}mcp${sep}tools`))
